...
namespace LIB_COMHelper
{
  ...
  HRESULT CComUtil::ExtractWindowMember(CComPtr<IWebBrowser2> &spBrowser, LPCOLESTR memberName, CComPtr<IDispatch> &result)
  {
    CIDispatchHelper window = CComUtil::IWebBrowserToIHTMLWindow(spBrowser);
    return window.Get<CComPtr<IDispatch>, VT_DISPATCH, IDispatch *>((LPOLESTR)memberName, result);
  }
}